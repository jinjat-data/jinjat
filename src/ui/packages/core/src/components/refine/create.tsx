...
export const JinjatCreate: React.FC<JinjatFormProps> = ({packageName, resources, title, logo, ...props}) => {
    const createResource = `_analysis/${packageName}.${resources.create}`;
    const {formLoading, onFinish} = useForm({resource: createResource});

    const [data, setData] = useState<object>({})
    const [errors, setErrors] = useState<ErrorObject[] | null>(null)

    const resource = `${packageName}.${resources.create}`;
    ...
    return <Create title={title} goBack={logo} isLoading={formLoading}
        ...
        {schema.data?.schema != null ?
            <JinjatForm data={data} schema={schema.data?.schema} onChange={setData} onError={setErrors} {...(props || {})}/> : null}
    </Create>
}
